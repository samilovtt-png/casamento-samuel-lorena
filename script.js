...
/* V35 — correção: modal de doação por projeto inicializado após o HTML estar pronto */
(function(){
  function initProjectDonationV35(){
    const modal = document.getElementById('projectModal');
    const form = document.getElementById('donationForm');
    const buttons = document.querySelectorAll('.project-detail-btn');
    if(!modal || !form || !buttons.length) return;

    const projectData = {
      passagens:{
        title:'Duas passagens missionárias',
        text:'Este projeto ajudará a custear duas passagens para que a missão continue atravessando fronteiras, alcançando comunidades e levando serviço, cuidado e esperança.',
        goal:'Meta: R$ 20.000'
      },
      container:{
        title:'Contêiner para as missões',
        text:'O contêiner será usado como estrutura de apoio para materiais, organização e recursos destinados a novos campos e ações missionárias.',
        goal:'Meta: R$ 40.000'
      },
      guine:{
        title:'Salas e igreja em Guiné-Bissau',
        text:'O objetivo é construir salas e um espaço de igreja para acolher, ensinar, servir a comunidade e criar um ponto permanente de esperança.',
        goal:'Meta: R$ 60.000'
      }
    };
    ...
    const goal = document.getElementById('projectModalGoal');
    const projectKey = document.getElementById('donationProjectKey');
    const nameInput = document.getElementById('donationName');
    const amountInput = document.getElementById('donationAmount');
    const copyBtn = document.getElementById('donationCopyPix');
    ...
    function openProject(key){
      ...
      if(goal) goal.textContent = d.goal;
      if(projectKey) projectKey.value = key;
      if(feedback){ feedback.textContent=''; feedback.className='donation-feedback'; }
      modal.classList.add('open');
      modal.setAttribute('aria-hidden','false');
      document.body.style.overflow='hidden';
    ...
    buttons.forEach(btn=>{
      if(btn.dataset.projectV35Bound === '1') return;
      btn.dataset.projectV35Bound = '1';
      btn.addEventListener('click', ()=>openProject(btn.dataset.project));
    });
    if(closeBtn && closeBtn.dataset.projectV35Bound !== '1'){
      closeBtn.dataset.projectV35Bound='1';
      closeBtn.addEventListener('click', closeProject);
    }
    if(modal.dataset.projectV35Bound !== '1'){
      modal.dataset.projectV35Bound='1';
    ...
    if(copyBtn && copyBtn.dataset.projectV35Bound !== '1'){
      copyBtn.dataset.projectV35Bound='1';
      copyBtn.addEventListener('click', async ()=>{
        try{ await navigator.clipboard.writeText(PIX_PAYLOAD); }
        catch(e){
          const ta=document.createElement('textarea');
          ta.value=PIX_PAYLOAD; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); ta.remove();
        }
        copyBtn.textContent='Pix copiado ✓';
        setTimeout(()=>copyBtn.textContent='Copiar Pix para doar',1600);
      });
    ...
    if(form.dataset.projectV35Bound !== '1'){
      form.dataset.projectV35Bound='1';
      form.addEventListener('submit', async e=>{
        e.preventDefault();
        const valor=Number(amountInput?.value||0);
        const nome=String(nameInput?.value||'').trim();
        const projeto=String(title?.textContent||'').trim();
        const key=String(projectKey?.value||'');
        if(!nome || valor<=0 || !projeto){
          if(feedback){feedback.textContent='Informe seu nome e o valor da doação.';feedback.className='donation-feedback error';}
          return;
        }
        const id='DOA-'+Date.now().toString(36).toUpperCase()+'-'+Math.random().toString(36).slice(2,6).toUpperCase();
        const payload={tipo:'doacao',id,nome,valor,projeto,projetoKey:key,status:'Informada pelo convidado',origem:'Site do casamento',rsvpId:localStorage.getItem('samuelLorenaLastRsvpId')||''};
        ...
        try{
          ...
          const data=await res.json().catch(()=>({}));
          if(!res.ok || !data.ok) throw new Error(data.error||'Falha ao registrar');
          localStorage.setItem('samuelLorenaLastDonationId',id);
          if(feedback){feedback.textContent=`Doação registrada para “${projeto}”. Muito obrigado!`;feedback.className='donation-feedback ok';}
          if(nameInput) nameInput.value='';
          if(amountInput) amountInput.value='';
        }catch(err){
          console.error('Doação:',err);
          if(feedback){feedback.textContent='Não foi possível registrar agora. Tente novamente.';feedback.className='donation-feedback error';}
        }finally{
          if(submitBtn){submitBtn.disabled=false;submitBtn.textContent='Registrar minha doação';}
        }
      });
    }
  }

  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initProjectDonationV35);
  else initProjectDonationV35();
})();
