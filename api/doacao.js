module.exports = async function handler(req, res) {
  ...
  try {
    const body = req.body || {};
    if (!body.nome || !body.projeto || Number(body.valor || 0) <= 0) {
      return res.status(400).json({ok:false,error:'Dados da doação inválidos'});
    }
    body.tipo = 'doacao';
    ...
    console.error(e);
    return res.status(502).json({ok:false,error:'Falha ao gravar doação na planilha'});
  }
}
